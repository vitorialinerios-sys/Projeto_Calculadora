...
# ============================================================
# GERAÇÃO DE GRÁFICO COM GNUPLOT
# ============================================================
...
if ! command -v gnuplot > /dev/null 2>&1; then
    ...
    echo "Instale com:"
    echo "sudo apt install gnuplot-nox"
    exit 1
fi
...
plot \
    "$CSV_FILE_PATH" every ::1 using 2:6 \
    with linespoints linewidth 2 pointtype 7 \
    title "Média da manhã", \
\
    "$CSV_FILE_PATH" every ::1 using 2:9 \
    with linespoints linewidth 2 pointtype 5 \
    title "Média da tarde", \
\
    "$CSV_FILE_PATH" every ::1 using 2:10 \
    with linespoints linewidth 2 pointtype 9 \
    title "Média do dia", \
\
    "$CSV_FILE_PATH" every ::1 using 2:11 \
    with lines linewidth 2 dashtype 2 \
    title "Média padrão"
EOF
